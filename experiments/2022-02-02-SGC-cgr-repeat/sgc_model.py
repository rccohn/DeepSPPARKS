...
class SGCNet(torch.nn.Module):
    def __init__(
        ...
        # SGCNet only has one layer, the SGConv layer multiple iterations of message
        # passing handled with K parameter from pyg documentation: cached should only
        # be set to true for transductive learning (ie where the same graph is used
        # and only the labels of some nodes are unknown)
        self.conv = SGConv(
    ...
    def _log(self):
        set_tags({"model_name": self.model_name})
        log_params(
            {
                "k": self.k,
            }
...
def train_step(model, batch, optimizer, mask):
    ...
    mask: torch.Tensor [N-node] element boolean tensor. If mask[n] == True, loss from
    prediction on node n is used to update gradients during training. Otherwise,
    the prediction is ignored during training.
    ...
    # torch.nn.functional.nll_loss(input, target) input: NxC double tensor. input[i,
    # j] contains the log-probability (ie log-softmax) prediction for class j of
    # sample i target: N-element int tensor N[i] gives the ground truth class label
    # for sample i forward() needs all nodes for message passing however,
    # when computing loss, we only consider nodes included in mask to avoid including
    # unwanted nodes (ie validation/test nodes, non-candidate grains, etc)
...
def train_loop(
    ...
    ta, tl = mean_acc_and_loss(
        model, data_train, data_train.candidate_mask
    )  # train accuracy, loss
    va, vl = mean_acc_and_loss(
    ...
    best_iter = 0
    msg = (
        "iteration {:>4d}/{:<4d} Train acc: {:.4f}, Val acc: {:.4f}, Train loss: {"
        ":.4f}, Val loss: {:.4f} "
    )
    ...
    for train_iter in t:
        ...
        # if validation loss is lower than previous best, update best val loss and
        # model params
        if vl < best_val_loss:
    ...
    cmlist = [confusion_matrix(gt, pred) for gt, pred in zip((gtt, gtv), (ypt, ypv))]
...
def mean_acc_and_loss(model, data, mask):
    ...
    ----------
        ...
        mask: torch.Tensor [N-node] element boolean tensor. If mask[n] == True,
        loss from prediction on node n is included in accuracy and loss calculations.
    ...
    """
    model.train(False)
    # note: avoid using model.predict() to avoid calling forward twice (we also need
