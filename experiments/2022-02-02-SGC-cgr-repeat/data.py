...
class Dataset:
    def __init__(
        self, name, raw_root=RAW_DATA_ROOT, processed_root=PROCESSED_DATA_ROOT, log=True
    ):
    ...
    def process(self, force=False):
        ...
        for subset in keys:
            ...
            print(
                "processing {} ({} files)".format(
                    raw.absolute(), len(list(raw.glob("*.json")))
                )
            )
            ...
            for f in files:
                ...
                # won't use more than 4 iterations of message passing, only need
                # subgraph with r=4 only 1 candidate grain per graph, with node index
                # g.cidx[0]
            ...
            for n in nli:
                ...
                bitmask = roll_img(
                    decode(node["rle"]), 1.0
                )  # get grain mask, and center in image
                # compute features from rprops
                rprops = regionprops_table(bitmask.astype(np.uint8), properties=props)
                feat = np.zeros(len(props) + 2)

                # first 2 features are grain type (1 for candidate, 0.5 for red
                # grain, 0 for blue grain) and number of neighbors (node degree)
                feat[0] = [1, 0, 0.5][
                    node["grain_type"]
            ...
            candidate_mask = np.zeros(
                len(g.nodes), bool
            )  # mask with True value at index of candidate grains
            ...
            subsets[subset] = Data(
                x=x, edge_index=edge_index, y=y, candidate_mask=candidate_mask
            )

        # mean and standard deviation for normalization note that we skip the first
        # column (grain type) as it is categorical and already bound between 0 and 1
    ...
    def _log(self):
        ...
        Note that it is assumed that a run has already been started. If not, mlflow
        will start a new one with default parameters, which is usually not desirable.

        Returns
        -------
            None

        Logs ------ dataset name, feature name, target name saved as tags in mlflow
        tracking server
        """
        mlflow.set_tags(
            {
