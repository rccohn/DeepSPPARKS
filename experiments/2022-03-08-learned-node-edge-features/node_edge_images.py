...
def roll_shifts(img: np.ndarray, i: Optional[Union[int, float]] = None) -> Tuple[int]:
    ...
    i: optional numeric
        if None, img should be a binary integer image. Shifts for mask with value 1
            will be computed.
        if specified, binary image will be determined by applying img == i,
            and then the shifts will be computed the samy way mentioned above
...
def extract_edge_patch(
    g: Graph, e: Tuple[int], box_size: int = 48, width: int = 2
) -> dict:
...
def edge_patch_to_image(
    patch: dict, edge_offset: float = 0.1, mp: Tuple[int] = (0.8, 0.4, 0.6)
) -> np.ndarray:
    ...
    for i in range(2):
        ...
        # highlight pixels on boundary. This way, the boundary is highlighted even
        # if the two connected grains have the same type
