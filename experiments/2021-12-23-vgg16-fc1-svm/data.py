...
# TODO tensorflow gpu does not work in current docker container, migrate
#     to torch to reduce dependency complexity
...
class Dataset:
    def __init__(
        ...
        self.processed_root = Path(
            processed_root
        )  # root directory to save processed data in
        ...
        self.crop = crop  # whether image will be cropped during processing to focus
        # on candidate grain
    ...
    def process(
        self, vgg16_path=None, force=False, log_featurizer=False, artifact_path=None
    ):
        keys = ("train", "val", "test")
        ...
        if not force:  # if force == True, skip this step and always process files
            ...
            try:  # load existing data
                ...
                return
            except AssertionError:  # if any files are not found, re-process whole
                # dataset
        ...
        # preprocess inputs: normalize by rgb mean [0.485 0.456 0.406] and standard
        # deviation [0.229, 0.224, 0.225], or equivalent based on format of images
        fc1_extractor = Model(
            inputs=vgg16.inputs, outputs=vgg16.get_layer("fc1").output
        )
        ...
        mapper = {
            (255, 255, 255): 1.0,  # candidate grain: highest signal
        ...
        for subset in keys:  # train, val, test
            ...
            checkpoint = (
                ...
                n // 2,
                n - 1,
            )  # index to save sample images for verification
            ...
            for i, f in enumerate(files):  # train/graph_xxx.json
                ...
                for k, v in mapper.items():  # each color in image
                    # [1 x 1 x 3 array] so mapper can match with all color channels
                    # for single pixel
                    map_array = np.array(k)[np.newaxis, np.newaxis, :]
                    mask = np.all(
                        img == map_array, axis=2
                    )  # True where all color channels match for given pixel
                    imgray[mask] = v
                if self.crop:  # 'zoom in' to get to 224x224
                    imgray = crop_img(imgray, 224, 224)
                else:  # resize image to get to 224x224
                    imgray = skimage.transform.resize(
                        imgray, (224, 224)
                    )  # imagenet size for vgg16
                imgray = skimage.img_as_ubyte(
                    imgray
                )  # vgg16 input expects ubyte images (pixels are int (0,255))
                if i in checkpoint:
                    savepath = Path(
                        artifact_path,
                        "{}-{}-{}-processed.png".format(subset, i, f.stem),
                    )
                ...
                imgray = np.expand_dims(
                    np.stack([imgray for _ in range(3)], axis=2), axis=0
                )
