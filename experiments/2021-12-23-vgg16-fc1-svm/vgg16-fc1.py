...
def main():
    ...
    with mlflow.start_run(nested=False):
        ...
        results_all = []  # concatenating all results for more convenient analysis of all child runs.
        # of course, if space becomes an issue, then you can just store the individual child results
        # and then concatenate them later outside of artifact storage.
            ...
            for whiten, whiten_label in enumerate(('no_whiten', 'whiten')):
                ...
                for cgr_thresh in params['cgr_thresh']:
                    with mlflow.start_run(run_name='vgg16-svm-trial', nested=True):
                        ...
                        results_header = ('n_components', 'var',  'svm_c', 'train_acc', 'val_acc',)
                        results = []
                        best_n_components = 0
                        best_val_acc = -1.
                        best_train_acc = -1.
        ...
        fname = artifact / 'pca-{}.joblib'.format((1-best_whiten_all)*'no_'+'whiten')
        ...
        mlflow.set_tags({
            'mlflow.runName': 'candidate-grains-vgg16',  # set run name manually, since mlflow project cannot set it
            'dataset': dataset.dataset_name,
            'features': dataset.feature_name,
            'targets': dataset.target_name,
            'model': 'sklearn-svm'
