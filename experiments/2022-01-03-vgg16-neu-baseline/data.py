...
class Dataset:
    def __init__(
        ...
        self.labels_fwd = {
            y: x for x, y in enumerate(self.labels_inv)
        }  # maps str label to int
        ...
        self._processed_path = Path(
            self.processed_root, self.dataset_name, self.feature_name, self.target_name
        )
    ...
    def process(
        ...
        data = {"files": [x.name for x in files], "X": features, "y": targets}
