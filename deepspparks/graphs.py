...
class Graph(nx.DiGraph):
    ...
    def cidx(self):
        """
        Index of candidate grains (grain type == 0). 
        
        Returns 
        -------
        cidx: list(int) list of node indices
    ...
    def get_metadata(self, n=None):
        ...
        ----------
        n: int or None
            Optional index of node to return
    ...
    def to_image(self, color_by_type=False, flip_horizontal=False, center_grain=None):
        """
        Converts graph to an image of the initial state of the SPPARKS simulation.
        
        # if both color_by_type and match_spparks are True, and center_grain=False,
        the image should match the first frame of the grains.mov output generated by
        spparks/meso.
        ...
        color by type: bool
            Default: False
            If True, add black edges to grains and coloreach grain by its 'type'
            If False, each pixel is simply labeled with the integer ID of the grain
            it is associated with.
        
        flip horizontal: bool
            default: False
            if True, reverses order of each column of pixels (mirror image)
            This was used during testing to verify that the original state of the
            SPPARKS system was reconstructed exactly.
        
        center_grain: optional, int or None
        if None, the image is returned without any shifts.
        If an integer is specified, the grain with the integer idx is centered in the image.
        

        Returns
        -------
        img: ndarray
            image of the initial state of the system
            with shape (r,c), or (r,c,3) if color_by_type==True
    ...
    def from_spparks_out(path):
        r"""
        Create graph from raw outputs from spparks. Wrapper for
        create_graph() function so it can be called as a class method.
    ...
    def to_spparks_out(self):
        """
        Used in previous testing, reconstruct SPPARKS output files from graph
        to verify that information is preserved exactly. However, after
        significant changes to code base, this function needs to be updated
        before use with testing (not needed for normal use).
        """
    ...
    def validate_with_spparks_outputs(self, root="", verbose=True):
        """
        Depricated, used in previous testing, but no longer needed.
    ...
    def to_dict(self):
        ...
        ----------- 
        jd: dict
            json-compatible dictionary with keys 'nodes',
